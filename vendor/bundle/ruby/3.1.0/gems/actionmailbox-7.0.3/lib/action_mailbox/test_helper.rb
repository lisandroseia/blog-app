...
module ActionMailbox
  module TestHelper
    ...
    def create_inbound_email_from_fixture(fixture_name, status: :processing)
      create_inbound_email_from_source file_fixture(fixture_name).read, status:
    end
    ...
    def create_inbound_email_from_mail(status: :processing, **mail_options, &block)
      ...
      create_inbound_email_from_source mail.to_s, status:
    end

    # Create an InboundEmail using the raw rfc822 +source+ as text.
    def create_inbound_email_from_source(source, status: :processing)
      ActionMailbox::InboundEmail.create_and_extract_message_id! source, status:
    end
