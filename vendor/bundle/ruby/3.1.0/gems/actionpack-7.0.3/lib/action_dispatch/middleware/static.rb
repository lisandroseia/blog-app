...
module ActionDispatch
  ...
  class Static
    def initialize(app, path, index: 'index', headers: {})
      @app = app
      @file_handler = FileHandler.new(path, index:, headers:)
    end
  ...
  class FileHandler
    ...
    def initialize(root, index: 'index', headers: {}, precompressed: %i[br
                                                                        gzip], compressible_content_types: %r{\A(?:text/|application/javascript)})
      @root = root.chomp('/').b
      @index = index
    ...
    def attempt(env)
      ...
      if (request.get? || request.head?) && found = find_file(request.path_info,
    ...
    def serve(request, filepath, content_headers)
      ...
      @file_server.call(request.env).tap do |status, headers, _body|
        # Omit Content-Encoding/Type/etc headers for 304 Not Modified
        headers.update(content_headers) if status != 304
      end
    ensure
    ...
    def find_file(path_info, accept_encoding:)
      each_candidate_filepath(path_info) do |filepath, content_type|
        if response = try_files(filepath, content_type, accept_encoding:)
          return response
        end
      end
    end
