...
module ActionController
  ...
  class Parameters
    ...
    def select(&)
      new_instance_with_inherited_permitted_status(@parameters.select(&))
    end
    ...
    def reject(&)
      new_instance_with_inherited_permitted_status(@parameters.reject(&))
    end
    ...
    def each_nested_attribute
      hash = self.class.new
      each { |k, v| hash[k] = yield v if Parameters.nested_attribute?(k, v) }
      hash
    end
    ...
    def specify_numeric_keys?(filter)
      filter.keys.any? { |key| /\A-?\d+\z/.match?(key) } if filter.respond_to?(:keys)
    end

    def each_element(object, filter, &)
      case object
      when Array
        object.grep(Parameters).filter_map(&)
      when Parameters
        if object.nested_attributes? && !specify_numeric_keys?(filter)
          object.each_nested_attribute(&)
        else
          yield object
        end
      end
    ...
    def permitted_scalar_filter(params, permitted_key)
      ...
      params[permitted_key] = self[permitted_key] if has_key?(permitted_key) && permitted_scalar?(self[permitted_key])
